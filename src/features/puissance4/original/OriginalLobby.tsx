...
export function OriginalLobby() {
  ...
  const handleBots = (bots: BotSetup) => {
    if (!requireName()) return;
    savePseudo(name);
    navigate(`/puissance4-original/${botRoomCode()}`, { state: { isHost: true, name: name.trim(), bots, mode } });
  };
  ...
  return (
    <div className="container p4-lobby">
      ...
      <header className="p4-lobby-header">
        <GameTitle as="h1" game="puissance4-original" size="xl" motto />
        <p>Le classique : aligne quatre jetons, sans pouvoirs, sur une grille 7 × 6.</p>
      ...
      <div className="p4-lobby-name">
        <label htmlFor="p4o-pseudo">Ton pseudo</label>
        <input
          id="p4o-pseudo"
          type="text"
          maxLength={18}
      ...
      <section className="p4-mode-picker">
        ...
        <div className="p4-mode-grid" role="radiogroup" aria-label="Mode de jeu">
          {MODE_ORDER.map((id) => {
            const config = ORIGINAL_MODES[id];
      ...
      <div className="p4-lobby-grid">
        <BotPlayCard className="p4-lobby-card" fixedCount={ORIGINAL_MODES[mode].players - 1} fixedNote={mode === 'teams' ? 'ton coéquipier est un bot' : 'selon le mode choisi plus haut'} onPlay={handleBots} />
